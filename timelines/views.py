"""
Views for looking at UserTimelineItem items by date.
...
They all also take an argument ``queryset`` which should be an ``UserTimelineItem``
queryset; it'll be used as the *starting point* for the the view in question
instead of ``UserTimelineItem.objects.all()``.
"""
...
def archive(request, queryset=None,
    ...
    Templates: ``timelines/archive.html`` (default)
    Context:
        ``items``
            Items, earliest first.
        ``date_list``
            List of years that have UserTimelineItems.
        ``all_content_types``
    ...
    if not queryset:
        queryset = UserTimelineItem.objects.all()
...
def year(request, year, queryset=None,
    template_name="timelines/year.html", template_loader=loader,
    extra_context=None, context_processors=None, mimetype=None, username=None):
    """
    UserTimelineItems for a particular year.
    ...
    Templates: ``timelines/year.html`` (default)
    Context:
        ``items``
    ...
    year = int(year)
    try:
        first = UserTimelineItem.objects.order_by("timestamp")[0]
    except IndexError:
...
def month(request, year, month, queryset=None,
    template_name="timelines/month.html", template_loader=loader,
    extra_context=None, context_processors=None, mimetype=None, username=None):
    """
    UserTimelineItems for a particular month.
    ...
    Templates: ``timelines/month.html`` (default)
    Context:
        ``items``
...
def day(request, year, month, day, queryset=None, recent_first=True,
    template_name="timelines/day.html", template_loader=loader,
    extra_context=None, context_processors=None, mimetype=None, username=None):
    """
    UserTimelineItems for a particular day.
    ...
    Templates: ``timelines/day.html`` (default)
    Context:
        ``items``
