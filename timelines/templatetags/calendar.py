...
@register.inclusion_tag('timelines/snippets/calendar.html', takes_context=True)
def calendar(context, year=date.today().year, month=date.today().month, active_day=date.today().day):
    """ Inclusion tag which renders the template timelines/snippets/calendar.html to build a calendar. """
    today = date.today()
